...
# We'll visualize the setup for a longer taper length to see the cone clearly
Lt = 8.0 
...
# Second, define a cone of air to create the hollow "empty conus"
# This cone is placed inside the solid taper, effectively subtracting itself.
hollow_cone = mp.Cone(
    center=mp.Vector3(0, 0, 0),
    height=Lt,
    radius=0.0,  # Starts at a point
    radius2=(w1 / 2.0) * 0.8, # Ends with a radius slightly smaller than the narrow waveguide
    axis=mp.Vector3(x=1), # Aligned along the x-axis
    material=mp.air
)

# The final geometry is the combination of the solid part and the air hole
geometry = [solid_taper, hollow_cone]
...
if __name__ == '__main__':
    plt.figure(dpi=150)
    sim.plot2D()
    plt.title(f"Setup with Hollow Conical Taper (Lt = {Lt} μm)")
    plt.show()
