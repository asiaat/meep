"""
Meep simulation script: Cylindrical-symmetric (2D axisymmetric) band-limited Hellwarth–Nouchi-like toroidal pulse in 0.5–10 GHz.

Fixes:
1. sim.meep_time() is called as a method.
2. sim.get_field_point may return complex values; we take the real part.
3. Fixed MacOS FigureCanvas issue: use `tostring_argb` + convert to RGB instead of `tostring_rgb`.
"""
...
def probe_cb(sim):
    ...
    probe_field.append(float(np.real(sim.get_field_point(mp.Ez, probe_pt))))
...
# Save & plot
# -------------------------
np.save(os.path.join(outdir, "probe_time.npy"), np.array(probe_time, dtype=float))
np.save(os.path.join(outdir, "probe_ez.npy"), np.array(probe_field, dtype=float))
...
if frames:
    ...
    for f in frames:
        ...
        fig.canvas.draw()
        # MacOS fix: use tostring_argb and convert to RGB
        buf = np.frombuffer(fig.canvas.tostring_argb(), dtype=np.uint8)
        buf = buf.reshape(fig.canvas.get_width_height()[::-1] + (4,))
        rgb = buf[:,:,[1,2,3]]  # drop alpha, reorder to RGB
        images.append(rgb)
        plt.close(fig)
